...
class RestaurantTest extends TestCase
{
    ...
    #[Test]
    public function test_name_and_address_setters_persist_initial_translations_in_primary_language(): void
    {
        $restaurant = Restaurant::factory()->create(['primary_language' => 'vi']);

        $restaurant->name = 'Quán Phở';
        $restaurant->address = '123 Nguyễn Huệ, Quận 1';
        $restaurant->save();

        $fresh = $restaurant->fresh();

        $this->assertSame('Quán Phở', $fresh->translate('name', 'vi'));
        $this->assertSame('123 Nguyễn Huệ, Quận 1', $fresh->translate('address', 'vi'));
        $this->assertTrue($fresh->translations()->where('is_initial', true)->where('locale', 'vi')->exists());
    }

    #[Test]
    public function test_name_and_address_are_writable_via_mass_assignment(): void
    {
        ...
        $fresh = $restaurant->fresh();
        $this->assertSame('Mass Assigned', $fresh->translate('name', 'en'));
        $this->assertSame('99 Some Street', $fresh->translate('address', 'en'));
    }

    #[Test]
    public function test_updating_name_replaces_existing_initial_translation(): void
    {
        $restaurant = Restaurant::factory()->create(['primary_language' => 'en']);
        ...
        $fresh = $restaurant->fresh();
        $this->assertSame('New Name', $fresh->translate('name', 'en'));
        $this->assertSame(
            1,
            $fresh->translations()->whereHas('translationField', fn ($q) => $q->where('name', 'name'))->count(),
        );
    }
}
